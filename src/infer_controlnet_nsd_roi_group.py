...
def get_args():
    ...
    # True ROI-group ablation: mask condition channels BEFORE roi_mixer / ControlNet.
    # Channel layout (from dataset_nsd_cond.py):
    #   channel 0 = Early ROI  (V1 & V2)
    #   channel 1 = Middle ROI (V3 & hV4)
    #   channel 2 = Late ROI   (FFA, LOC, etc.)
    ap.add_argument("--roi_group_mode", type=str, default="full",
                    choices=[
                        "full",                        # keep all three ROI groups
                        "early_only",                  # zero out Mid + Late
                        "mid_only",                    # zero out Early + Late
                        "late_only",                   # zero out Early + Mid
                        "no_early",                    # zero out Early,  keep Mid + Late
                        "no_mid",                      # zero out Mid,    keep Early + Late
                        "no_late",                     # zero out Late,   keep Early + Mid
                    ],
                    help=(
                        "ROI-group ablation: mask condition channels BEFORE roi_mixer/ControlNet.\n"
                        "  full       : all ROI groups active (default)\n"
                        "  early_only : only Early (V1/V2) active\n"
                        "  mid_only   : only Middle (V3/hV4) active\n"
                        "  late_only  : only Late (FFA/LOC) active\n"
                        "  no_early   : Middle + Late active\n"
                        "  no_mid     : Early + Late active\n"
                        "  no_late    : Early + Middle active\n"
                    ))

    # Depth ablation: mask ControlNet residuals AFTER ControlNet.
    ap.add_argument("--roi_mode", type=str, default="full",
                    choices=["full", "early", "mid", "late"],
                    help=(
                        "Depth ablation on ControlNet residuals (applied AFTER ControlNet forward pass).\n"
                        "  full  : keep all residual depths (default)\n"
                        "  early : keep only shallow residual band\n"
                        "  mid   : keep only middle residual band\n"
                        "  late  : keep only deep residual band + mid block\n"
                    ))
...
def apply_roi_group_mode(cond: torch.Tensor, mode: str) -> torch.Tensor:
    """
    Apply ROI-group ablation by zeroing condition channels BEFORE roi_mixer / ControlNet.

    Channel layout:
      cond[:, 0] = Early ROI  (V1 & V2)
      cond[:, 1] = Middle ROI (V3 & hV4)
      cond[:, 2] = Late ROI   (FFA, LOC, etc.)

    This must be called before roi_mixer so that cross-attention
    cannot recover information from the zeroed-out groups.
    """
    if mode == "full":
        return cond

    if cond.ndim != 4 or cond.shape[1] < 3:
        raise ValueError(
            f"Expected cond_values with shape [B, C>=3, H, W], "
            f"but got {tuple(cond.shape)}. Check condmap channel layout."
        )
    ...
    if mode == "early_only":
        cond[:, 1, :, :] = 0.0   # zero Mid
        cond[:, 2, :, :] = 0.0   # zero Late

    elif mode == "mid_only":
        cond[:, 0, :, :] = 0.0   # zero Early
        cond[:, 2, :, :] = 0.0   # zero Late
    ...
    # --- two-group (leave-one-out) modes ---
    elif mode == "no_early":
        cond[:, 0, :, :] = 0.0   # zero Early, keep Mid + Late

    elif mode == "no_mid":
        cond[:, 1, :, :] = 0.0   # zero Mid, keep Early + Late

    elif mode == "no_late":
        cond[:, 2, :, :] = 0.0   # zero Late, keep Early + Mid

    else:
        raise ValueError(f"Unknown roi_group_mode: {mode!r}")
...
def apply_roi_mode(down_block_res, mid_block_res, roi_mode: str):
    """
    Depth ablation: keep only one depth-band of ControlNet residuals.
      early -> shallow band
      mid   -> middle band
      late  -> deep band + mid block residual
      full  -> keep everything (no-op)
    """
    if roi_mode == "full":
        return down_block_res, mid_block_res
    ...
    mid_out = mid_block_res if roi_mode == "late" else (
        torch.zeros_like(mid_block_res) if mid_block_res is not None else None
    )
...
def main():
    ...
    print(f"[infer] roi_group_mode={args.roi_group_mode}")
    print(f"[infer] depth roi_mode={args.roi_mode}")
    ...
    for batch in loader:
        ...
        # ── ROI-group ablation ──────────────────────────────────────────────
        # Must happen BEFORE roi_mixer so cross-attention cannot
        # recover zeroed-out ROI information from the other channels.
        cond = apply_roi_group_mode(cond, args.roi_group_mode)
        ...
        encoder_hidden_states = prepare_text_embeddings(
            tokenizer, text_encoder, prompts, device, do_cfg
        ...
        for t in scheduler.timesteps:
            ...
            down_block_res, mid_block_res = scale_controlnet_residuals(
                down_block_res, mid_block_res, depth_scales
            )
            # ── Depth ablation (kept separate from ROI-group ablation) ──────
            down_block_res, mid_block_res = apply_roi_mode(
                down_block_res, mid_block_res, args.roi_mode
            )
        ...
        for b in range(len(images)):
            ...
            meta = dict(
                ...
                roi_group_mode=args.roi_group_mode,
