...
class NSDAccess(object):
    ...
    def read_betas(self, subject, session_index, trial_index=[], data_type='betas_fithrf_GLMdenoise_RR', data_format='func1pt8mm', mask=None):
    ...
    def read_atlas_results(self, subject, atlas='HCP_MMP1', data_format='fsaverage'):
        ...
        mapp_df = pd.read_csv(os.path.join(self.nsddata_folder, 'freesurfer', 'fsaverage',
                                           'label', f'{atlas_name}.mgz'), delimiter=' ', header=None, index_col=0)
        ...
        if data_format not in ('func1pt8mm', 'func1mm', 'MNI'):
            # if surface based results by exclusionasz
